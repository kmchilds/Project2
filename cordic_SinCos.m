...
%theta  = fi(thNorm, 1, sumWL, sumFrac); % Fixed-point angle values (best precision)

z_NT   = numerictype(theta);             % Data type for Z
xyNT   = numerictype(1, sumWL, sumFrac); % Data type for X-Y
...
for idx = 1:length(theta)
    ...
    [x_out(idx), y_out(idx), z_out(idx)] = cordic_rotation_kernel(...
